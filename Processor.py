...
class Processor():
    ...
    def load_weights_from_srlstm(self):
        if self.args.pretrain_load > 0:
            ...
            if os.path.isfile(self.args.model_save_path):
                ...
                self.net.gcn.relativeLayer.MLP[0].weight.data = nn.Parameter(
                    saved_weights['gcn.relativeLayer.MLP.0.weight'])
                self.net.gcn.relativeLayer.MLP[0].bias.data = nn.Parameter(
                    saved_weights['gcn.relativeLayer.MLP.0.bias'])
    ...
    def load_model(self):
        if int(self.args.load_model) > 0:
            self.args.model_save_path = self.args.save_dir + '/' + self.args.train_model + '/' + self.args.train_model + '_' + \
                                        str(self.args.load_model) + '.tar'
            if os.path.isfile(self.args.model_save_path):
                print('Loading checkpoint')
                checkpoint = torch.load(self.args.model_save_path, map_location='cuda:0')
                model_epoch = checkpoint['epoch']
                self.net.load_state_dict(checkpoint['state_dict'])
                debug(f'Loaded checkpoint at epoch {model_epoch}')
    ...
    def playtest(self):
        ...
        print('Set: {}, epoch: {}, test_error: {:.5f} test_final_error: {:.5f}'.format(self.args.test_set,
    ...
    def train_epoch(self, epoch):
        ...
        for batch in range(self.dataloader.trainbatchnums):
            ...
            inputs_fw = batch_abs[:-1], batch_norm[:-1], shift_value[:-1], seq_list[:-1], nei_list[:-1], nei_num[
                                                                                                         :-1], batch_pednum[
                                                                                                               :-1]
            ...
            lossmask, num = get_loss_masks(outputs, seq_list[0], seq_list[1:], using_cuda=self.args.using_cuda)
            ...
            if batch % self.args.show_step == 0 and self.args.ifshow_detail:
                print(
                    'train-{}/{} (epoch {}), train_loss = {:.5f}, time/batch = {:.5f}, value1={:.5f},value2={:.5f},value3={:.5f}'.format(
    ...
    def val_epoch(self, epoch):
        ...
        for batch in range(self.dataloader.valbatchnums):
            ...
            lossmask, num = get_loss_masks(outputs_infer, seq_list[0], seq_list[1:], using_cuda=self.args.using_cuda)
    ...
    def test_epoch(self, epoch):
        ...
        for batch in range(self.dataloader.testbatchnums):
            ...
            inputs, batch_key_list = self.dataloader.get_test_batch(batch)
            # debug(f'batch_key_list={batch_key_list}', kill=True)
            ...
            seq_batch, seq_batch_translated, translations, seq_data_mask_batch, \
            seq_adjacency_matrix_batch, seq_neighbor_num_batch, ped_num_batch, \
            seq_pedids_batch, seq_start_frames_batch = inputs
            inputs_fw = seq_batch[:-1], seq_batch_translated[:-1], translations[:-1], seq_data_mask_batch[:-1], \
                        seq_adjacency_matrix_batch[:-1], seq_neighbor_num_batch[:-1], ped_num_batch[:-1]
            forward = self.net.forward
            self.net.zero_grad()
            output_seq, _, _, look = forward(inputs_fw, iftest=True)

            # debug(f'seq_batch shape {seq_batch.shape}, output_seq shape {output_seq.shape}', kill=True)
            ...
            loss_masks, num = get_loss_masks(output_seq, seq_data_mask_batch[0], seq_data_mask_batch[1:],
                                             using_cuda=self.args.using_cuda)
            error, error_cnt, final_error, final_error_cnt, error_nl, error_nl_cnt, _ = \
                call_errors(output_seq,
                            seq_batch_translated[1:, :, :2],
                            self.args.obs_length,
                            loss_masks,
                            seq_data_mask_batch[1:],
                            nl_thred=0)

            col = cal_col_rate(output_seq, translations[1:], seq_batch[0, :, :], seq_batch[1:, :, :2],
                                seq_pedids_batch[1:], seq_start_frames_batch[1:],
                                loss_masks, batch_key_list)
            col_rates = col_rates + col
        ...
        debug(f'\nTotal col rate = {col_rate}', kill=True)
